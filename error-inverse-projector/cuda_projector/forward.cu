...
renderCUDA(
	...
	// the effective extent is now depended on the opacity of gaussian.
			...
			const float input[5] = {s.x / cutoff, s.y / cutoff, in_dirs[collected_id[j]].x, in_dirs[collected_id[j]].y, in_dirs[collected_id[j]].z};
			float computed_color[3];
			// Global mem version
			siren_forward(
				W1, b1, W2, b2, collected_id[j], input, computed_color
			);
